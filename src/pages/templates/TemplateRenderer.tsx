...
import { createTemplate, getTemplateById } from "@/api/TemplateService";
...
function TemplateRenderer() {
   ...
   async function onSubmit(data: any) {
    ...
    if (loading) {
        return (
            <Layout>
                <div className="p-6">Loading template...</div>
            </Layout>
        );
    }
                    ...
                    <CardHeader>
                        {/* <CardTitle className="text-2xl">{template.title}</CardTitle> */}
                         <div className="flex flex-col sm:flex-row sm:items-end gap-4 pt-2">
                             <FormField control={form.control} name="title" render={({ field }) => ( <FormItem className="flex-grow"> <FormLabel>Template Title</FormLabel> <FormControl><Input {...field} /></FormControl> <FormMessage /> </FormItem> )} />
